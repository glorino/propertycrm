...
async function processIncomingWhatsApp(from: string, text: string) {
  ...
  let ticketNumber = `PCR-${Date.now().toString().slice(-6)}`;
...
• Your order number (starts with # or PCR-)
...
We'd love to show you how PropertyCRM can transform your property support, ${customerName}!
...
Have a wonderful day! ✨`;
  ...
  if (fuzzyMatch(lower, ["hello", "hi", "hey", "good morning", "good afternoon", "good evening", "helo", "heloo", "hii", "heythere"])) {
    return `Hello ${customerName}! 👋 Welcome to PropertyCRM Support.
